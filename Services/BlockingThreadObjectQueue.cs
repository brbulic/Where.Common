...
namespace Where.Common.Services
{
	...
	public class BlockingThreadObjectQueue<T>
	{
		...
		public BlockingThreadObjectQueue(Action<T> operationAction, Action<T> onFail, Func<bool> runnerPredicate, Func<T> processData = null, int timeout = 20)
		{
		...
		public Guid EnqueueOperation(T operation)
		{
			lock (_queueLock)
			{
		...
		{
			Func<T, bool> linqDelegate = data => byKey(data);

			lock (_queueLock)
			{

				return _operationsQueue.Any(linqDelegate);
			}
		}
		...
		public bool CancelOperation(Predicate<T> byKey)
		{
			...
			lock (_queueLock)
			{
				gotEm = _operationsQueue.Where(linqDelegate).ToList();
			}
		...
		public void StartOperation()
		{
			...
			_runnerThread.Name = string.Format("BlockingThreadObjectQueue-{0}", _runnerThread.ManagedThreadId);
			_runnerThread.Start();
		}
